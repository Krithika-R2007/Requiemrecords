...
class UIManager {
    constructor() {
        this.containers = {
            ...
            seance: document.getElementById('seance-container'),
            summoning: document.getElementById('summoning-container'),
            ghostEncounter: document.getElementById('ghost-encounter-container')
        };
        ...
        try {
            ...
            try {
                this.elements.logEntries.innerHTML = entries.map(entry => {
                    ...
                    // Get transcription text (with fallback)
                    const transcription = entry.transcription || '[No transcription available]';
                    const hasTranscription = entry.transcription && !entry.transcription.includes('[') && entry.transcription.length > 0;
                    ...
                    return `
                        <div class="entry-card" data-id="${entry.id}" role="listitem" aria-label="Memory entry from ${entry.dateFormatted || 'Unknown date'}">
                            ...
                            <div class="entry-controls" role="group" aria-label="Playback controls">
                                ...
                                <button class="control-button play-spooky" data-id="${entry.id}" aria-label="Generate spooky story">Spooky Story</button>
                                <button class="control-button delete-entry" data-id="${entry.id}" aria-label="Delete this memory">Delete</button>
                            </div>
                            <div class="entry-transcription" data-id="${entry.id}" role="region" aria-label="Transcription">
    ...
    showSeance() {
        this.hideAll();
        this.containers.seance.classList.remove('hidden');
        
        // Play séance video
        const seanceVideo = document.getElementById('seance-video');
        if (seanceVideo) {
            seanceVideo.play().catch(err => {
                console.error('Failed to play séance video:', err);
            });
        }
    }

    /**
     * Show summoning state
     */
    showSummoning() {
        this.hideAll();
        this.containers.summoning.classList.remove('hidden');
        
        // Play summoning video
        const summoningVideo = document.getElementById('summoning-video');
        if (summoningVideo) {
            summoningVideo.play().catch(err => {
                console.error('Failed to play summoning video:', err);
            });
        }
    }

    /**
     * Show ghost encounter
     * @param {Object} ghost - Ghost data
     */
    showGhostEncounter(ghost) {
        this.hideAll();
        this.containers.ghostEncounter.classList.remove('hidden');
        
        // Set ghost image
        const ghostImage = document.getElementById('ghost-image');
        if (ghostImage) {
            ghostImage.src = `assets/ghosts/${ghost.image}`;
            ghostImage.alt = ghost.name;
        }
        
        // Set ghost overlay color
        const ghostOverlay = document.getElementById('ghost-overlay');
        if (ghostOverlay) {
            ghostOverlay.style.setProperty('--ghost-color', ghost.color);
        }
        
        // Set ghost info
        const ghostName = document.getElementById('ghost-name');
        const ghostRarity = document.getElementById('ghost-rarity');
        const dangerValue = document.getElementById('danger-value');
        const dangerBars = document.getElementById('danger-bars');
        
        if (ghostName) ghostName.textContent = ghost.name;
        if (ghostRarity) ghostRarity.textContent = ghost.probability;
        
        // Set danger level
        const dangerDesc = this.getDangerDescription(ghost.dangerLevel);
        const dangerColor = this.getDangerColor(ghost.dangerLevel);
        
        if (dangerValue) {
            dangerValue.textContent = dangerDesc;
            dangerValue.style.color = dangerColor;
            dangerValue.style.setProperty('--danger-color', dangerColor);
        }
        ...
        if (dangerBars) {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                if (i < ghost.dangerLevel) {
                    bar.classList.add('filled');
                    bar.style.setProperty('--danger-color', dangerColor);
                }
                dangerBars.appendChild(bar);
            }
        }
        
        // Reset ghost dialogue to show initial prompt (IMPORTANT FIX!)
        const ghostDialogue = document.getElementById('ghost-dialogue');
        if (ghostDialogue) {
            // Restore original structure with buttons
            ghostDialogue.innerHTML = `
                <p class="ghost-greeting" id="ghost-greeting">${ghost.greeting}</p>
                <p class="history-prompt" id="history-prompt">${ghost.historyPrompt}</p>
            ...
            // Set CSS variable for ghost color
            ghostDialogue.style.setProperty('--ghost-color', ghost.color);
            
            // Re-attach event listeners (since we replaced the HTML)
            const revealBtn = document.getElementById('reveal-history-btn');
            const declineBtn = document.getElementById('decline-history-btn');
            
            if (revealBtn) {
                revealBtn.addEventListener('click', () => {
                    // Get the app controller from window
                    if (window.hauntedVinylApp) {
                        window.hauntedVinylApp.handleRevealHistory();
                    }
                });
            }
            ...
            if (declineBtn) {
                declineBtn.addEventListener('click', () => {
                    // Get the app controller from window
                    if (window.hauntedVinylApp) {
                        window.hauntedVinylApp.handleDeclineHistory();
                    }
                });
            }
        }
        
        // Set CSS variables for ghost color
        const ghostInfoPanel = document.getElementById('ghost-info-panel');
        
        if (ghostInfoPanel) {
            ghostInfoPanel.style.setProperty('--ghost-color', ghost.color);
        }
        
        // Add danger-level-based visual effects to container
        const encounterContainer = document.getElementById('ghost-encounter-container');
        if (encounterContainer) {
            // Remove existing danger classes
            encounterContainer.classList.remove('danger-high', 'danger-extreme');
            
            // Add appropriate danger class
            if (ghost.dangerLevel >= 9) {
                encounterContainer.classList.add('danger-extreme');
            } else if (ghost.dangerLevel >= 7) {
                encounterContainer.classList.add('danger-high');
            }
        }
    }
     ...
     * @param {Object} ghost - Ghost data
     */
    showGhostHistory(ghost) {
        const ghostDialogue = document.getElementById('ghost-dialogue');
        if (!ghostDialogue) return;
        
        // Replace dialogue with spooky history text
        ghostDialogue.innerHTML = `
            <p class="ghost-history-text">${ghost.history}</p>
            <button class="close-encounter-button" id="close-history-btn">Return to the séance</button>
        `;
        
        // Maintain ghost color
        ghostDialogue.style.setProperty('--ghost-color', ghost.color);
        ...
        if (closeBtn) {
            closeBtn.addEventListener('click', () => {
                ...
                const event = new CustomEvent('closeGhostEncounter');
    ...
    /**
     * Show idle warning effects
     */
    showIdleWarning() {
        const vignette = document.getElementById('vignette-effect');
        const mainContent = document.querySelector('#ghost-encounter-container .ghost-background');
        
        if (vignette) vignette.classList.add('active');
        if (mainContent) mainContent.classList.add('blur-effect', 'active');
    }
    ...
    hideIdleWarning() {
        ...
        if (mainContent) mainContent.classList.remove('blur-effect', 'active');
    }

    /**
     * Show séance timeout effects (screen darkens, user gets kicked out)
     */
    showSeanceTimeout() {
